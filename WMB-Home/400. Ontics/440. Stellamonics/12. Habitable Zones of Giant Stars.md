...
> [!important]
> Aldebaran’s true mass is measured at $1.16 M⊙$​.  If, however, we incorrectly applied the main-sequence radius–mass scaling, we’d get $M \approx 67.34⊙$.  The main-sequence [[01. Stars and Spectral Classes|luminosity–mass scaling]] would give $M \approx 4.959⊙$​. Both are wildly wrong — illustrating why Main Sequence relations must only be applied to Main Sequence stars; using them on evolved stars like Aldebaran gives wildly misleading results.

Using Aldebaran's correct (measured) mass value, its perannual orbit falls at $A AU, which is only $0.845$ AU (1.26 million kilometers) beyond the star’s surface.
...
>Remember:
>Such hypergiants live only a few million years at most, far too short for Terran-like biospheres to develop. (See [[14. Lifetime, Life, and Habitability]] for details.)
